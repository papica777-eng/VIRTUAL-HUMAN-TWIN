...
function HealingPolicySelector() {
  const enabled = useNexusStore((s) => s.autoHealing.enabled);
  const policy = useNexusStore((s) => s.autoHealing.policy);
  const toggle = useNexusStore((s) => s.toggleAutoHealing);
  const setPolicy = useNexusStore((s) => s.setHealingPolicy);
